...
export default async function RezervasyonBasarisizPage({
  ...
  const whatsappHref = WHATSAPP
    ? `https://wa.me/${WHATSAPP.replace(/\D/g, '')}?text=${encodeURIComponent(`Merhaba, ödeme işlemim başarısız oldu. Rezervasyon no: ${ref}`)}`
    : null
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="mt-2 text-sm text-slate-500 text-center max-w-xs">
            {isPaid
              ? 'Kayıtlarımızda ödemeniz alınmış görünüyor. Tutarsızlık varsa lütfen bizimle iletişime geçin.'
              : 'Ödeme işleminiz başarısız sonuçlandı. Kart bilgilerinizi kontrol ederek tekrar deneyebilirsiniz.'}
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          {!isPaid && (
            <div className="flex items-start gap-3 border-b border-amber-100 bg-amber-50 px-5 py-4">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                ...
                Tekrar denemek için geri dönebilirsiniz.
              </p>
            </div>
          )}
          ...
          <div className="divide-y divide-slate-100 px-5">
            <DetailRow label="Rezervasyon no" value={<span className="font-mono font-semibold">{ref}</span>} />
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50/60 via-slate-50 to-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="mt-5 text-2xl font-bold text-slate-900 text-center">Ödeme tamamlanamadı</h1>
          <p className="mt-2 text-sm text-slate-500 text-center max-w-xs">
            Kartınızdan herhangi bir çekim yapılmamış olabilir. Tekrar denemek için geri dönün.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="flex flex-col gap-3 p-5">
            ...
            <Link
              href="/"
              className="flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"
            >
              Ana sayfaya dön
            </Link>
          </div>
...
// FailPageBackButton, NoDataFail bileşeninde de kullanıldığı için burada import etmemiz gerekmiyor
// (zaten dosya başında import edildi)
